package com.lemon.rpcframe.core.cluster;
...
import com.lemon.rpcframe.util.JSONUtils;
...
public class Config implements Serializable {
    ...
    private static final long serialVersionUID = -1027386961355261891L;
    // 节点是否可用
    private boolean available = true;

    // 应用节点组
    private String nodeGroup = "dtask";

    // 唯一标识
    private String identity = StringUtils.generateUUID();

    // 工作线程, 目前只对 TaskTracker 有效
    private int workThreads = Constants.AVAILABLE_PROCESSOR;

    // 节点类型
    private NodeType nodeType;

    // 注册中心 地址
    private String registryAddress = "zookeeper://192.168.120.189:2181";

    // 远程连接超时时间
    private int invokeTimeoutMillis = 1000 * 60;

    // 监听端口
    private int listenPort = Constants.JOB_TRACKER_DEFAULT_LISTEN_PORT;

    // 任务信息存储路径(譬如TaskTracker反馈任务信息给JobTracker, JobTracker down掉了,
    // 那么存储下来等待JobTracker可用时再发送)
    private String failStorePath = Constants.USER_HOME;
